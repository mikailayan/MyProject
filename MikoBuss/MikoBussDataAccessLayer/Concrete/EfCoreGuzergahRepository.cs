        ...
        public List<Guzergah> GetBySelectedGuzergahList(string nereden, string nereye, DateTime tarih)
        {
            using (var context = new MikoBussContext())
            {
                ...
                var nereyee = context
                    ...
                    .Select(i => i.CiytName)
                    .FirstOrDefault();
                string tarihh = tarih.ToString("dd-MM-yyyy");
                var xx = context
                    .Guzergahs
                    .Where(i => i.GuzergahStart == Convert.ToString(neredenn) && i.GuzergahEnd == Convert.ToString(nereyee) && i.GuzergahTarihi== tarihh)
                    .ToList();
                return xx;
            }
        }
